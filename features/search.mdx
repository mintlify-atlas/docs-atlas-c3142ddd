...
The search input is visible in the navbar on desktop viewports. On mobile it is hidden in the collapsed navbar by default.
...
The `index.json` template (`layouts/_default/index.json`) iterates over all pages recursively and serializes each one as a JSON object:

```yaml
# Each entry in index.json contains:
# - title       — the page title
# - description — the page description param
# - content     — full rendered HTML content
# - image       — the featured image param
# - permalink   — absolute URL to the page
```

The raw template that produces this output is:
